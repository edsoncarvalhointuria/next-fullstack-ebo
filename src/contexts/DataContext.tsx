...
interface ProviderDataContext {
    ...
    addCongregacao: (congregacao: CongregacaoInterface | CongregacaoInterface[]) => void;
    addComprador: (comprador: CompradorInterface) => void;
    addCargo: (cargo: CargosInterface | CargosInterface[]) => void;
    addCheckin: (checkin: { id: number; id_credencial: number; id_usuario: string; data_hora_checkin: string }) => void;
    addPergunta: (pergunta: FAQInterface | FAQInterface[]) => void;
    addTransacao: (transacao: TransacaoInterface) => void;
    addCredencial: (transacao: TransacaoInterface | CredencialInterface[]) => void;
}
...
export default function DataContext({ children }: { children: ReactNode }) {
    ...
    const responsesMemo = useMemo(() => {
        ...
        const credenciaisCongregacoesMap = new Map<string, GraficoCredenciaisIgrejas>();
        const credenciaisCargosMap = new Map<string, GraficoCredenciaisCargos>();
        const credenciaisMap = new Map<string, CredenciaisTotal>();

        const credenciaisResponse: CredencialResponse[] = credenciais.map((v, i) => {
            const {
                id_cargo,
                id_congregacao,
                id,
            ...
            const transacao = transacoes.find((v) => v.id === id_transacao)!;
            const comprador = compradores.find((v) => v.cpf_cnpj === transacao.id_comprador)!;

            const ingresso = ingressos.find((v) => String(v.id) === String(transacao.id_ingresso))?.nome_tipo!;
            const cargo = testeCargos.find((c) => String(c.id) === String(id_cargo))?.nome!;
            const congregacao = testeCongregacoes.find((c) => String(c.id) === String(id_congregacao))?.nome!;
            congregacoesSet.add(id_congregacao);

            const objIgreja = credenciaisCongregacoesMap.get(congregacao) || {
        ...
        const listaGraficosTransacoes: GraficosTransacoes[] = transacoes.map((v) => {
            const {
                data_hora_pedido,
                id,
                id_ingresso,
            ...
            const data = new Date(data_hora_pedido).toLocaleDateString("pt-BR", { day: "2-digit", month: "2-digit" });

            const tipo_ingresso = ingressos.find((v) => String(v.id) === String(id_ingresso))?.nome_tipo || "";
            const credencial = credenciais.find((v) => v.id_transacao === id && v.is_titular)!;

            const titular_congregacao =
                congregacoes.find((v) => String(v.id) === String(credencial?.id_congregacao))?.nome || "";

            const titular_cargo = cargos.find((v) => String(v.id) === String(credencial?.id_cargo))?.nome || "";
            const comprador = compradores.find((v) => v.cpf_cnpj === id_comprador)!;
            ...
            if (status_pagamento === "aprovado") {
                ...
                tipo_ingresso === "individual" ? transacoesPorIngresso.individual++ : transacoesPorIngresso.casal++;
            ...
            responseTransacoes.push({
                ...
                credenciais:
                    credenciaisLista?.map((v) => {
                        const { is_outra_congregacao, nome, nome_outra_congregacao, nome_cargo, nome_congregacao } = v;
            ...
            (objCredenciais as any)[tipo_ingresso] = ((objCredenciais as any)[tipo_ingresso] || 0) + 1;

            credenciaisMap.set(data, objCredenciais);

            return {
                data_pedido: new Date(data_hora_pedido).toLocaleDateString("pt-BR", {
                    day: "2-digit",
                    month: "2-digit",
                }),
                id_transacao: id,
        ...
        const transacoesArrecadacao = Object.values(objArrecadacao);
        // .sort((a: any, b: any) => {
        ...
        // });
        const credenciaisPorCongregacaoOrdenada = Array.from(credenciaisCongregacoesMap.values()).sort(
            (a, b) => b.quantidadeCredenciais - a.quantidadeCredenciais,
        );
        const credenciaisPorCongregacao = credenciaisPorCongregacaoOrdenada.map((v) => ({
            name: v.name,
            inscritos: v.quantidadeCredenciais,
        }));
    ...
    }, [transacoes, credenciais, ingressos, cargos, congregacoes, compradores, checkins]);
    const addIngresso = useCallback((ingresso: IngressosInterface | IngressosInterface[]) => {
        setIngressos((v) => ("length" in ingresso ? ingresso : [...v, ingresso]));
    }, []);
    const addCongregacao = useCallback((congregacao: CongregacaoInterface | CongregacaoInterface[]) => {
        setCongregacoes((v) => ("length" in congregacao ? congregacao : [...v, congregacao]));
    }, []);
    const addComprador = useCallback((comprador: CompradorInterface) => {
        setCompradores((v) => [...v, comprador]);
    }, []);
    const addCargo = useCallback((cargo: CargosInterface | CargosInterface[]) => {
        setCargos((v) => ("length" in cargo ? cargo : [...v, cargo]));
    }, []);
    const addCheckin = useCallback(
        (checkin: { id: number; id_credencial: number; id_usuario: string; data_hora_checkin: string }) => {
            setCheckins((v) => [...v, checkin]);
        },
        [],
    );
    const addPergunta = useCallback((pergunta: FAQInterface | FAQInterface[]) => {
        setPerguntas((v) => ("length" in pergunta ? pergunta : [...v, pergunta]).sort((a, b) => a.ordem - b.ordem));
    }, []);
    const addTransacao = useCallback((transacao: TransacaoInterface) => {
        setTransacoes((v) => [...v, transacao]);
    }, []);
    const addCredencial = useCallback((credencial: CredencialInterface | CredencialInterface[]) => {
        setCredenciais((v) => ("length" in credencial ? credencial : [...v, credencial]));
    }, []);
